...
test("Search: Mylist search data validation", async ({ page, mockApi, enableSearchPage }) => {
    await mockApi()
    ...
    expect(href).toContain("mylist/0") // マイリストID: 0
...
test("Search: Mylist search sort options validation", async ({ page, mockApi, enableSearchPage }) => {
    await mockApi()
    await enableSearchPage()
