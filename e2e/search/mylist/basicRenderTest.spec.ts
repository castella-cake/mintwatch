...
test("Search: Basic mylist search rendering test", async ({ page, mockApi, enableSearchPage }) => {
    await mockApi()
    await enableSearchPage()

    await page.goto("https://www.nicovideo.jp/mylist_search/TEST")
    ...
    // 検索結果のマイリストタイトルが正しく表示されているか確認
    const mylistTitle = await page.locator(".info-card-content-title").first().textContent()
    expect(mylistTitle).toBe("TestMylist 0")
    ...
    // 検索フォームにマイリスト検索が選択されていることを確認
    const activeSearchType = page.locator(".searchbox-type-active[data-searchtype='mylist']")
