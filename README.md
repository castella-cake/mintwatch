...
### 望み通りになれるカスタマイズ性
旧視聴ページに合わせたレイアウトにしたり、新視聴ページに合わせたレイアウトにしたり。  
MintWatch のプレイヤーテーマが気に入らなければ HTML5 プレイヤーのプレイヤーテーマを選んだり。  
それぞれの使い方に合わせた設定ができ、どのレイアウトに慣れていた人でも、すぐに視聴を始められます。

### 過去を振り返る
原宿バージョンの視聴ページを現代テイストにアレンジして再現した Shinjuku を実装しています。
また、プレイヤーテーマからはFlashプレイヤーを再現した GINZA+ を利用できます。

システム面でも、過去ログの読み込みと、Flash時代のCA互換を利用できます。

### ちょっと便利になる機能を使う
ボカコレ用の歌詞を表示したり、Misskey や Bluesky への共有、  
大百科可用性表示の復活など、通常の視聴ページにはない機能を備えています。

### UserStyle/UserScriptフレンドリー
ニコニコ動画の新視聴ページは、"現代的な"CSSクラスの命名によりUserStyle/UserScriptを使用したカスタマイズが難しくなっています。   
MintWatch は、新視聴ページ、そしてTailwindやCSS Modulesを使用せずにページを構築しており、よりカスタマイズ可能なオープンソースの視聴ページを目指して開発されています。   
