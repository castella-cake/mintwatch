...
これはタグ編集ボタンをクリックして閉じる操作を自動化したようなものですが、それでも自己責任でお願いします。
...
また、詳細設定から「職人モード」をONにすると、
隠しコマンドもパレットから入力できるようになります。どうぞご活用ください。

### コメント詳細の改善
コメントリストから時間表示をクリックして開ける、コメントの詳細表示でできることが増えました。

これからは直接そのユーザーをNGしたり、自身が投稿したコメントの削除を行えるようになります。  
動画IDを含んでいる場合はその動画へのリンクが表示され、職人モードが有効の場合はコメントのソースが表示されるようになります。
...
このアップデートに伴って、以前の設定では少し音量が下がって聞こえます。  
お手数をおかけしますが、再度音量設定の調整をお願いします。
...
- 冬の間だけサイドメニューを開くと雪が降るようになりました (詳細設定から無効化できます)
