...
export function NavigationDndWrapper({ children }: { children: ReactNode }) {
    ...
    function handleDragEnd(e: DragEndEvent) {
        ...
        const currentSettings = navBarCustomItemList ?? ["recommendations", "history", "ranking"]
        ...
        if (thisActiveId.startsWith("sidemenu-")) {
            ...
            if (thisOverId === "navbar-droppable") {
                storage.setItem("sync:navBarCustomItemList", [...currentSettings, thisActiveId.replace("sidemenu-", "")])
                ...
                storage.setItem("sync:navBarCustomItemList", afterArray)
                setCurrentDraggingItem(null)
            }
            ...
            storage.setItem("sync:navBarCustomItemList", sortAfter)
            setCurrentDraggingItem(null)
        } else if (e.active.id.toString().startsWith("navbar-")) {
            storage.setItem("sync:navBarCustomItemList", currentSettings.filter((item: string) => item !== thisActiveId.replace("navbar-", "")))
            setCurrentDraggingItem(null)
        }
    }
