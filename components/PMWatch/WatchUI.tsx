...
function isValidSeriesVideoItem(value: unknown): value is SeriesVideoItem {
    return (
        ...
        Object.prototype.hasOwnProperty.call(value, 'requireSensitiveMasking') &&
...
function CardDragOverlay({ draggingItem }: { draggingItem: unknown }) {
    if ( isValidRecommendItem(draggingItem) ) {
        const thisVideoId = (draggingItem && draggingItem.id) || ( draggingItem && draggingItem.content && draggingItem.content.id ) || null
        return draggingItem && draggingItem.content ? <Card
            thumbnailUrl={draggingItem.content.thumbnail && draggingItem.content.thumbnail.listingUrl}
            thumbText={draggingItem.content.duration ? secondsToTime(draggingItem.content.duration) : "??:??"}
...
function CreateWatchUI() {
    ...
    const [ videoActionModalState, setVideoActionModalState ] = useState<false | "mylist" | "share" | "help">(false)
    ...
    const [currentDraggingItem, setCurrentDraggingItem] = useState<unknown | null>(null)
    ...
    function onModalStateChanged(isModalOpen: boolean, modalType: "mylist" | "share" | "help") {
    ...
    function handleDragEnd(e: DragEndEvent) {
        ...
        } else if ( e.over && e.active.id.toString().includes("-series") && e.active.data.current ) {
    ...
    return <div className={isFullscreenUi ? "container fullscreen" : "container"} >
        {(videoInfo.data) && <title>{videoInfo.data.response.video.title} - ニコニコ動画</title>}
        <CSSTransition nodeRef={onboardingPopupElemRef} in={!localStorage.playersettings.onboardingIgnored && !isFullscreenUi} timeout={300} unmountOnExit classNames="pmw-onboarding-popup-transition">
            <OnboardingPopup onOnboardOpen={() => {onModalStateChanged(true, "help")}} nodeRef={onboardingPopupElemRef}/>
        </CSSTransition>
        ...
        <div className="watch-container" is-bigview={shouldUseBigView ? "true" : "false"} watch-type={layoutType} settings-size={playerSize} use-card-recommend={shouldUseCardRecommend} use-horizontal-search={shouldUseHorizontalSearchLayout} use-card-info={shouldUseCardInfo} id="pmw-container" onClickCapture={(e) => {linkClickHandler(e)}}>
            ...
            <CSSTransition nodeRef={videoActionModalElemRef} in={videoActionModalState !== false} timeout={300} unmountOnExit classNames="videoaction-modal-transition">
                <VideoActionModal nodeRef={videoActionModalElemRef} onModalStateChanged={onModalStateChanged} videoInfo={videoInfo} selectedType={videoActionModalState}/>
            </CSSTransition>
        </div>
        <DragOverlay>
