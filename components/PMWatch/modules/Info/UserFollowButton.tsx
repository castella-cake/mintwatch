...
export default function UserFollowButton({ userId }: { userId: string | number }) {
    ...
    if (isFollowingData === null) return <button className="videoinfo-owner-followbtn">
        <IconProgressHelp/>
        <br/>
        <span>ロード中</span>
    </button>

    return <button className="videoinfo-owner-followbtn" title={isFollowingData.following ? "フォロー解除" : "フォロー"} onClick={onFollowClick}>
