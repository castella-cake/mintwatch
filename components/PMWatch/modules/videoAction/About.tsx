...
export default function AboutMintWatch() {
        ...
        if (event.over && event.over.id === "droppableLogo" && progress === 2) {
            ...
            setTimeout(() => {
                showAlert({
                    ...
                    body: <div>
                        <p>
                            And now we present...<br/>
                            <br/>
                            nothing?<br/>
                        </p>
                        <p>
                            そうです。今はベータ期間です。<br/>
                            ベータテストに参加してくださった方の名前をここに乗せる予定です。<br/>
    ...
    return <div className="pmw-help-content about-mintwatch-content">
        <DndContext sensors={sensors} onDragOver={dragOverHandler} onDragEnd={dragEndHandler}>
            ...
            <p>
                MintWatch や PepperMint の支援者、使用しているライブラリの開発者やメンテナー、<br/>
                そしてユーザーベースの一員となってくださっているあなたに感謝を申し上げます。<br/>
            </p>
        </DndContext>
    </div>
}
...
function EasterDraggablePMFamilyLogo() {
    ...
    return <span ref={setNodeRef} style={style} {...attributes} {...listeners} className="about-draggable">
        <PMFamilyLogo/>
    </span>
}
...
function EasterDroppableHeader({ progress, onUnlock }: { progress: 0 | 1 | 2 | 3, onUnlock: (event: React.MouseEvent) => void }) {
    ...
    return <div className="about-mintwatch-header" style={progress >= 1 ? { position: "relative" } : {}}>
        <MintWatchLogo
