...
export function StatsOverlay({ videoInfo, hlsRef, videoRef }: { videoInfo: VideoDataRootObject | null, hlsRef: RefObject<Hls>, videoRef: RefObject<HTMLVideoElement | null> }) {
    ...
    return <div className="statsoverlay">
        ...
        <br/>
        現在のHLS再生クオリティ: { hlsRef.current && hlsRef.current?.levels[hlsLevel] ?  (hlsLevel === -1 ? <>Lv {hlsLevel} Auto</> : 
            <>
                ...
                {hlsRef.current?.levels[hlsLevel].bitrate / 1000}kbps (max {hlsRef.current?.levels[hlsLevel].maxBitrate / 1000} / avg {hlsRef.current?.levels[hlsLevel].averageBitrate / 1000})
            </>
        ) : <>内部HLS無効</>}
        <br/>
        <br/>
        ...
        {videoInfo.data?.response.media.domand?.videos.map(elem => {
            return <span key={`videoq-${elem.id}`}>
                {`ID: ${elem.id} - ${elem.width}x${elem.height} (${elem.label} / ${elem.bitRate / 1000}kbps)`}<br/>
            </span>;
        })}
        <br/>
        取得した音声クオリティ: <br/>
