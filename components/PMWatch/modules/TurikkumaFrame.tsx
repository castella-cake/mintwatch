...
export function TurikkumaFrame() {
    ...
    return (
        <div className="turikkuma-portal" id="pmw-turikkuma-iframe">
            { openMessageShown && (
                <div className="turikkuma-portal-message">
                    探索力のあるあなたに
                    <strong>「「つりっくま」」</strong>
                    をどうぞ…
                    <br />
                    (つりっくまのiframe を手に入れました)
                </div>
            ) }
            { syncStorage.turikkumaPortalOpened
                ? (
                        <iframe
                ...
                : (
                        <>
                            <CSSTransition in={!isRodClicked} timeout={500} nodeRef={rodButtonRef} classNames="turikkuma-portal-door-transition" unmountOnExit>
                                <button
                                    ref={rodButtonRef}
                                    className="turikkuma-portal-door"
                                    onClick={(e) => {
                                        if (e.ctrlKey) {
                                            setIsRodClicked(true)
                                        }
                                    }}
                                >
                                    🎣
